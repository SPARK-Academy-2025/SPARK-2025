...
def single_merge_masks(whole_tumor_nii, other_nii):
   whole_tumor = whole_tumor_nii.get_fdata()
   other = other_nii.get_fdata()

   output = np.where(whole_tumor == 1, 1, other)
   return output

def merge_masks(whole_tumor_dir: str, other_dir: str, output_dir: str):
    for file in sorted(os.listdir(whole_tumor_dir)):
        if ".DS_Store" in file: continue
        whole_tumor_nii = nib.load(f"{whole_tumor_dir}/{file}")
        other_nii = nib.load(f"{other_dir}/{file}")
        output_data = single_merge_masks(whole_tumor_nii, other_nii)
        output_nii = nib.Nifti1Image(output_data, affine=whole_tumor_nii.affine)
        nib.save(output_nii, f"{output_dir}/{file}")
    
    shutil.rmtree(whole_tumor_dir, ignore_errors=True)
...
class MyQuickTrainer(nnUNetTrainer):
    ...
    for file in sorted(os.listdir(f"{output_path}/non_mapped")):
       if not("BraTS" in file): continue
       case_name = f"{mapping[file[:-7]]}-seg.nii.gz"
